...
def extract_nodes(mxGraphModel, default_kind):
    ...
    for obj in mxGraphModel.findall(".//object"):
        ...
        labels = obj.get(
            "labels", None
        )  # Assuming 'labels' is stored directly; adjust if it's more complex
    ...
    for mxCell in mxGraphModel.findall(".//mxCell[@vertex='1']"):
        ...
        if mxCell.find("ancestor::object") is None and node_id not in node_details:
            ...
            if "image=data" not in style:
                continue
            if node_label:
                node_details[node_id] = {"label": node_label, "kind": default_kind}
        ...
        for object_elem in mxGraphModel.findall(".//object"):
            ...
            for mxCell in mxCells:
                ...
                link_info = extract_link_info(
                    mxCell, node_details, fallback_id=object_id
                )
                if link_info:
                    links_info[link_info["id"]] = link_info
...
def extract_link_info(mxCell, node_details, fallback_id=None):
    ...
    x, y = (
        (float(geometry.get(coord, 0)) for coord in ("x", "y"))
...
def aggregate_node_information(node_details):
    ...
    for node_id, details in node_details.items():
        ...
        if details.get("kind") and details.get("kind") != "nokia_srlinux":
            ...
            node_kind = (
                "linux"
                if "client" in details["label"]
                else details.get("kind", "nokia_srlinux")
            )
...
def compile_link_information(links_info, style="block"):
    ...
    for link_id, info in links_info.items():
        ...
        if len(sorted_labels) < 2:
            report_error(
                f"Not enough labels for link {link_id}. At least 2 labels are required."
            )
    ...
    compiled_links.sort(
        key=lambda x: (
            x["endpoints"][0].split(":")[0] if style == "block" else x["endpoints"]
        )
    )
...
def filter_nodes(links_info, node_details):
    ...
    involved_labels = {info["source"] for info in links_info.values()} | {
        info["target"] for info in links_info.values()
    }
    ...
    filtered_details = {
        node_id: details
        for node_id, details in node_details.items()
        if details["label"] in involved_labels
    }
    ...
    for node_id, details in filtered_node_details.items():
        ...
        if node_kind not in kinds:
            if node_kind == "nokia_srlinux":
                ...
                kinds[node_kind] = {
                    "image": None
                }  # Add more conditions as necessary for other kinds
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse a draw.io XML file and generate a YAML file with a specified style."
    )
    parser.add_argument(
        "-i",
        "--input",
        dest="input_file",
        required=True,
        help="The input XML file to be parsed.",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        choices=["block", "flow"],
        default="flow",
        help="The style for YAML endpoints. Choose 'block' or 'flow'. Default is 'block'.",
    )
    parser.add_argument(
        "--diagram-name",
        dest="diagram_name",
        required=False,
        help="The name of the diagram (tab) to be parsed.",
    )
    parser.add_argument(
        "--default-kind",
        dest="default_kind",
        default="nokia_srlinux",
        help="The default kind for nodes. Default is 'nokia_srlinux'.",
    )
